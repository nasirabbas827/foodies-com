...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Set the default status to 'pending' for all users
    $status = 'pending';
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err) && empty($user_type_err)) {
        ...
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssiss", $param_username, $param_password, $param_email, $param_phone, $param_age, $param_user_type, $status);
